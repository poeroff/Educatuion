...
import { L06C02A04 } from './store';

const P01 = () => {
  const subjectCode = import.meta.env.VITE_APP_CODE.toUpperCase();
  const pageKey = 'p01';
  const pageNo = 'P01';
  const cardPath = 'L06/C02/A04';
  const { changeData, initData, submitData, saveData } = usePageData();
  const pageIds = useRecoilValue(pageIdsAtom);
  const pageData = useRecoilValue(pageDataAtom);
  const [cardData, setCardData] = useRecoilState(L06C02A04);
  ...
  const data: IListenAndAnswer[] = [
    {
      ...
      content: (
        <Box>
          <Typography useGap={false}>What do you want to do on your birthday?</Typography>
        </Box>
      ),
      audioSrc: '/L06/C02/A04/ME1-L06-C02-A04-P01-1.mp3',
    },
    {
      type: 'B',
      ...
      content: (
        <Box>
          <Typography useGap={false}>I want to have a party with my friends.</Typography>
        </Box>
      ),
      audioSrc: '/L06/C02/A04/ME1-L06-C02-A04-P01-2.mp3',
    },
  ];
