...
const P02 = () => {
  ...
  const [cardData, setCardData] = useRecoilState(L07C02A06);
  ...
  const questionInfo = {
    text: `A가 되어 그림 속 여행지에 관해 더 묻는 문장을 완성한 후, 대화를 연습해 봅시다.`,
  };
  ...
  const data = {
    1: {
      ...
      content: (
        <>
          <Typography>Can you tell me more about</Typography>
          <Input
    ...
    2: {
      ...
      content: <Typography>Sure. It's in Venezuela.</Typography>,
    },
  };
  ...
  return (
    ...
    >
      <Box vAlign='center' hAlign='center' flexDirection='column'>
        <PinchZoom pinchType={'image'}>
          <Image alt={'바위 산에서 흘러 내리는 폭포수'} src={'/L07/C02/A06/ME1-L07-C02-A06-P02.jpg'} height='175px' />
        </PinchZoom>
        <Box background='#C0E4CB' borderRadius={'16px'} width={'280px'} textAlign='center' marginTop={'10px'}>
          <Typography weight={'var(--font-weight-medium)'} lineHeight='36px'>
            Angel Falls
          </Typography>
        </Box>
      </Box>
        ...
        <Box vAlign='center'>
          ...
          <Box width={'20%'} hAlign='flex-end' vAlign='flex-start'>
            {!isEmptyAudioData(cardData[pageKey].audioData?.[1]) ? (
              <SimpleAudioPlayer audioSrc={'/L07/C02/A06/ME1-L07-C02-A06-P02.mp3'} onEnded={handleAudioEnded} ref={audioRef} />
            ) : null}
          </Box>
        </Box>
      </Box>
