...
const P02 = () => {
  ...
  const questionInfo: IQuestionProps = {
    text: 'A가 되어 방문한 여행지를 알고 있는지 묻는 문장을 완성한 후, 대화를 연습해 봅시다.',
  };
  ...
  const data: IListenAndAnswer[] = [
    {
      ...
      content: (
        <Box vAlign='middle'>
          <Typography useGap={false}>Do you know about &nbsp;</Typography>
          <Input
            maxLength={50}
    ...
    },
    {
      ...
      content: (
        <Box>
          <Typography useGap={false}>I don't know much about it.</Typography>
        </Box>
      ),
      color: 'var(--color-orange-100)',
      audioSrc: '/L07/C02/A04/ME1-L07-C02-A04-P02.mp3',
    },
  ];
  ...
  return (
    <Container
      ...
      submitLabel='완료하기'
      submitBtnColor={
        cardData[pageKey]?.isSubmitted || !(cardData[pageKey]?.listeningData && cardData[pageKey]?.answer)
          ? EStyleButtonTypes.SECONDARY
          : EStyleButtonTypes.PRIMARY
      }
      submitDisabled={cardData[pageKey]?.isSubmitted || !(cardData[pageKey]?.listeningData && cardData[pageKey]?.answer)}
            ...
            <BoxWrap paddingBottom={40}>
              ...
              <Box width={'70%'}>{value?.content}</Box>
              {
                <Box width={'30%'} hAlign='flex-end' gap='6px'>
                  {(index === 1 || value?.conversationType === 'speak') && (
                    <Recorder
                      recorderIndex={3}
                  ...
                  {value?.conversationType === 'listen' && isRecordingEnd && (
                    <SimpleAudioPlayer audioSrc={value?.audioSrc ?? ''} ariaLabel='B번 지문 듣기 버튼' onEnded={handleListeningEnd} />
                  )}
                </Box>
              }
            </BoxWrap>
