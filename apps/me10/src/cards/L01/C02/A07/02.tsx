...
const P02 = () => {
  ...
  const data: IListenAndAnswer[] = [
    {
        ...
        <>
          <Box hAlign='start'>
            <Box padding='19px 11px'>
              <Label background='var(--color-blue-100)' shape='square' value={2} />
            </Box>
            <Box>
              <Box>
                <Typography>
      ...
      color: '',
      audioSrc: '/L01/C02/A07/ME1-L01-C02-A07-P02.mp3',
    },
  ];
  ...
  const checkDisableInput = () => {
    return (
      cardData.p02.answer?.some(val => val === '') ||
  ...
  const handleAudioEnded = (subKey: number) => {
    const newIsAudioPlayed = (cardData.p02.isAudioPlayed || []).map((v, i) => (i === subKey - 1 ? true : v));
    setCardData(prev => ({
      ...prev,
          ...
          {({ value, index = 1 }: { value?: IListenAndAnswer; index?: number }) => (
            <BoxWrap>
              ...
              <Box hAlign='flex-end'>
                ...
                <Recorder
                  recorderIndex={index}
                  initialData={cardData.p02.audioData}
