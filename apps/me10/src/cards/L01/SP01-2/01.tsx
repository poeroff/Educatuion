...
const P01 = () => {
  ...
  const initWordList = useMemo(() => {
    return cardData[pageKey].map((word: { word: string }) => ({ ...word, path: `/L01/SP01-1/M1_L1_voca_${word.word}.mp3` }));
  }, [pageIds]);

  const handleChange = (index: number, status: boolean) => {
    setCardData(prev => ({
      ...prev,
      [pageKey]: prev[pageKey].map((item: { id: number }) => (item.id === index ? { ...item, isMemorized: status } : item)),
    }));
  };
