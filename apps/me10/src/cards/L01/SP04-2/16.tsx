...
const P16 = ({ pageKey = DEFAULT_PAGE_KEY }: P16Props) => {
  ...
  const choices = ['has', 'have'];
  const wordNode = [
    <Typography key='wordNode' fontSize='var(--font-size-36)' weight='var(--font-weight-bold)'>
      We <Typography type='blank' width='70px' title='빈칸' boxColor='var(--color-black)' /> six lessons a day.
    </Typography>,
  ];
  ...
  const nodes: React.ReactNode[] = [
    <List
      ...
      gap={25}
      row={({ value, index = 1 }) => (
        <Box width='286px' textAlign='center' marginLeft={80} marginRight={80}>
          <Radio
            type='box'
            align='vertical'
            label={value}
