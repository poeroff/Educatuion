  ...
  const nodeData: IContentList[] = [
    {
      children: (
        <Box>
          <Typography>3. A beautiful park where people can relax lies {''}</Typography>
          <Typography textDecoration='underline' useGap={false} style={{ textUnderlinePosition: 'under' }}>
      ...
      children: (
        <BoxWrap>
          <Box hAlign={'flex-start'} marginTop='10px'>
            ...
            <Typography>In the heart of the city </Typography>
            <Input
              value={cardData[pageKey].answer}
              maxLength={70}
              onChange={hadleChange}
              ariaLabel={'In the heart of the city 이후에 들어갈 문장(단어)'}
