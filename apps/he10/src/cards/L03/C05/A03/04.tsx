...
const P04 = () => {
  ...
  const handleRadioClick = (index: number) => {
    setCardData(prev => ({ ...prev, p04: { ...prev.p04, answer: index.toString() } }));
    changeData('P04', 1, 1, index.toString());
  };
      ...
      <Box useFull hAlign='center' padding='10px 0px'>
        <Box useFull hAlign='center' flexDirection='column'>
          <Box width='926px' display='flex' justifyContent='center' padding='20px' background='white' useRound>
            Too much use of social media can <CustomUnderline width={'130px'} /> with studying.
          </Box>
        </Box>
      </Box>
      <BoxWrap>
        {imgs.map((item, index) => {
          ...
          return (
            <Radio
              ...
              value={adjustedIndex === Number(cardData.p04.answer)}
            ...
            >
              <ShadowedButton
                ...
                type='img'
                state={
                  adjustedIndex === Number(cardData.p04.answer)
                    ? cardData.p04.isSubmitted
                      ? adjustedIndex === Number(cardData.p04.solution)
