...
const P07 = () => {
  ...
  const handleSubmit = () => {
    if (!cardData.p07.isSubmitted) {
      const { answer } = cardData.p07;
      const isCorrect = isAnswer(answer, [cardData.p07.solution, cardData.p07.solution2]);
          ...
          inputData: [
            {
              ...
              isAnswer: true,
              isCorrect: isAnswer(answer, [cardData.p07.solution, cardData.p07.solution2]),
            },
          ],
          isCorrect,
        },
  ...
  const handleChangeAnswer = (event: ChangeEvent<HTMLInputElement>) => {
    setCardData(prev => ({ ...prev, p07: { ...prev.p07, answer: event.target.value } }));
      ...
      <Box display='flex' gap={'8px'}>
        <Box hAlign='center'>
          <PinchZoom pinchType={'image'}>
            <Image type={EImageType.IMG} src={'/L03/C07/A02/HE1-L03-C07-A02a-P07.jpg'} width='320px' height='378px' ariaDescribedby='img_desc' />
              ...
              <Box gap={'6px'} display='flex'>
                <Typography lineHeight='40px'>
                  ...
                  <Input
                    ...
                    maxLength={100}
                    ariaLabel={`서술 답안 입력란 9`}
                    status={
                      cardData.p07.isSubmitted
                        ? !isAnswer(cardData.p07.answer, [cardData.p07.solution, cardData.p07.solution2])
                          ? InputStatus.ERROR
                          : InputStatus.DEFAULT
                        : InputStatus.ENABLE
                    }
                    ...
                    width='180px'
                  />{' '}
                  sounds from people talking loudly near you
                </Typography>
              </Box>
            </Box>
          </Scroll>
        ...
        <Box background='lightGray' borderRadius='12px' marginTop='48px'>
          ...
          <Box marginTop='12px'>
            <Typography useGap={false} usePre>
              <Typography>
                9) {cardData.p07.solution} ({cardData.p07.solution2}도 정답 인정)
              </Typography>
            </Typography>
          </Box>
        </Box>
