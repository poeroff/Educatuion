...
세 번째 칸 Speakers에 관한 문장`;
  ...
  const handleSubmit = () => {
    if (!cardData.p05.isSubmitted) {
      const { answer } = cardData.p05;
      const isCorrect = isAnswer(answer, [cardData.p05.solution, cardData.p05.solution2]);
          ...
          inputData: [
            {
              ...
              isAnswer: true,
              isCorrect: isAnswer(answer, [cardData.p05.solution, cardData.p05.solution2]),
            },
          ],
          isCorrect,
        },
      ...
      <Box display='flex' gap={'8px'}>
        <Box hAlign='center'>
          <PinchZoom pinchType={'image'}>
            <Image type={EImageType.IMG} src={'/L03/C07/A02/HE1-L03-C07-A02a-P05.jpg'} width='320px' height='378px' alt={altText} />
          </PinchZoom>
        </Box>
            ...
            <Box width='100%' flexDirection='column' vAlign='center' gap={'12px'}>
              ...
              <Box gap={'6px'} display='flex'>
                <Typography lineHeight='40px'>
                  analyzes the sounds that the microphones detect and generates{' '}
                  ...
                  <Input
                    ...
                    maxLength={100}
                    ariaLabel={`서술 답안 입력란 7`}
                    status={
                      cardData.p05.isSubmitted
                        ? !isAnswer(cardData.p05.answer, [cardData.p05.solution, cardData.p05.solution2])
                          ? InputStatus.ERROR
                          : InputStatus.DEFAULT
                        : InputStatus.ENABLE
                    }
        ...
        <Box background='lightGray' borderRadius='12px' marginTop='48px'>
          ...
          <Box marginTop='12px'>
            <Typography useGap={false} usePre>
              <Typography>
                7) {cardData.p05.solution} ({cardData.p05.solution2}도 정답 인정)
              </Typography>
            </Typography>
          </Box>
        </Box>
