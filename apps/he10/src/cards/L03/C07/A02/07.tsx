...
import { textContentA02, imgContentA02P07 } from './commonData';
...
const P07 = () => {
  ...
  const { imgSrc, imgAlt } = imgContentA02P07;
  ...
  const handleDropdownClick = (index: number, newValue: string | undefined) => {
    ...
    const originalValue = cardData.p07.answer;
    const newValues = [...originalValue];

    if (newValue !== undefined) {
      newValues[index] = newValue;
      setCardData(prev => ({ ...prev, p07: { ...prev.p07, answer: newValues } }));
    }
  ...
  return (
    <Container
      ...
      submitBtnColor={
        cardData.p07.answer.every(isNotEmptyString) ? (isShow ? EStyleButtonTypes.GRAY : EStyleButtonTypes.PRIMARY) : EStyleButtonTypes.SECONDARY
      }
      submitLabel={cardData.p07.isSubmitted ? (isShow ? '답안닫기' : '답안보기') : '채점하기'}
      onSubmit={submitAnswer}
      submitDisabled={!cardData.p07.answer.every(isNotEmptyString)}
      vAlign={'flex-start'}
    >
        ...
        <Box marginLeft={'24px'} height={'342px'} width='50%' hAlign='center'>
          <Scroll height={'282px'} width={'434px'}>
            ...
            <Box hAlign='flex' display='inline-flex'>
              ...
              <Dropdown
                width='264px'
                dropdownList={cardData.p07.dropArr}
                isOpen={openDropdown[0]}
                selectedValue={cardData.p07.answer[0]}
                readOnly={cardData.p07.isSubmitted}
                onClick={value => handleDropdownClick(0, value)}
                ariaLabel='7번째 답 선택칸'
                isError={cardData.p07.isSubmitted && cardData.p07.answer[0] !== cardData.p07.solution[0]}
              />
            ...
            <Box display='inline'>
              <Typography weight={'normal'} size={EStyleFontSizes.MEDIUM}>
                from people talking loudly near you
              </Typography>
            </Box>
          </Scroll>
        </Box>
