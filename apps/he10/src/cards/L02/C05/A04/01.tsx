...
import { L02C05A04 } from './store';
import { useRecoilValue, useRecoilState } from 'recoil';

const P01 = () => {
  const pageKey = 'p01';
  const pageIds = useRecoilValue(pageIdsAtom);
  const [cardData, setCardData] = useRecoilState(L02C05A04);

  const initWordList = useMemo(() => {
    return cardData[pageKey].map(word => ({ ...word, path: `/L02/C05/A04/HE1-L02-C05-A04-${word.id.toString().padStart(2, '0')}.mp3` }));
  }, [pageIds]);
