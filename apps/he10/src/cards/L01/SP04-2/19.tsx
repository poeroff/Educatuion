...
const P19 = () => {
  ...
  return (
    <Container
      ...
      submitLabel={!cardData.p19.isSubmitted ? '채점하기' : !showAnswer ? '답안 보기' : '답안 닫기'}
      submitDisabled={!cardData.p19.isSubmitted && cardData.p19.answer === 0}
      submitBtnColor={cardData.p19.answer !== 0 ? (showAnswer ? EStyleButtonTypes.GRAY : EStyleButtonTypes.PRIMARY) : EStyleButtonTypes.SECONDARY}
    >
      <BoxWrap>
          ...
          <List gap={10} data={data}>
            {({ value, index = 1 }) => (
              <Radio
                ...
                defaultValue={index === cardData.p19.answer}
                onClick={() => handleChange(index)}
                disabled={cardData.p19.isSubmitted}
      ...
      <BottomSheet bottomSheetTargetId='targetContainer' height='40%' show={showAnswer}>
        <Box background='lightGray' borderRadius='12px' marginTop='48px'>
          ...
          <Box marginTop='12px'>{cardData.p19.solution}</Box>
          <Box marginTop='40px'>
