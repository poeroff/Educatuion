...
const P12 = ({ pageNumber = 'p12', store = 'SP01-1' }: P12Props) => {
  ...
  const audioInfo: IAudioPlayerProps = {
    audioSrc: '/L01/SP01-1/HE1-L01-SP01-1-P12.mp3',
  };

  const chipButtonInfo: IChipButtonInfo[] = useMemo(() => {
    return [
      {
        text: 'a better environment',
  ...
  const answer = 'providing a better environment to study and relax in';
  const solution = [4, 0, 3, 2, 1];
  ...
  return (
    ...
    >
      <Box marginBottom='24px' useRound height='137px' background='white' padding='10px'>
        <Box>
          <Typography>It now offers a larger and more comfortable space than before,</Typography>
        </Box>
        <Box>
          <Typography>
            <Typography type='blank' width='600px' title='빈칸' boxColor='var(--color-black)'></Typography>.
          </Typography>
        </Box>
      </Box>
