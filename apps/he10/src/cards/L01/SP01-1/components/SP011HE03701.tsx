...
import L01SP02State from '@/cards/L01/SP02/store';
...
interface IHE01401 {
  ...
  answer: string;
  pageNumber: string;
  store?: 'SP01-1' | 'SP01-2' | 'SP02';
}

const SP011HE03701 = ({ headerText, questionText, word, choices, answer, pageNumber, store = 'SP01-1' }: IHE01401) => {
  const { changeData, initData, submitDataWithResult, saveData } = usePageData();
  const pageIds = useRecoilValue(pageIdsAtom);
  const { userId } = useRecoilValue(studentAtom);
  const [cardData, setCardData] = useRecoilState(store === 'SP01-1' ? L01SP011State : store === 'SP01-2' ? L01SP012State : L01SP02State);
