...
export function A04() {
  ...
  const resetCardData = useResetRecoilState(L01C06A04b);
  ...
  useEffect(() => {
    ...
    const getIds = async () => {
      const list: number[] = await getPageId(`${appId}_L01-C06-A04b`);
  ...
  const info = [
    {
      ...
      altText: [``],
      udl: [
        `남자 선생님이 개와 늑대 사진 아래 각각의 설명이 적힌 인포그래픽 슬라이드를 설명하고 있다.`,
        `이미지 제목`,
        `Dogs vs. Wolves (Case 1)`,
        `슬라이드 텍스트`,
        `Dogs followed Dr. Hare’s Gestures -> found the cup with the food easily`,
        `Wolves paid no attention to his gestures -> struggled and chose cups randomly`,
      ],
      text: [
      ...
      imagePosition: 'before', // 'before', 'after', 'both', 'doubleAfter'
    },
  ];
  ...
  return (
    <Page selectedPage={selectedPage} setPage={setPage}>
      <HE01603 headerInfo={headerInfo} audioInfo={audioInfo} title={title} info={info} />
      <P02page />
