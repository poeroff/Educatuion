...
const P21 = ({ pageNumber = 'p21', store = 'SP03-2' }: P21Props) => {
  ...
  const chipButtonInfo: IChipButtonInfo[] = useMemo(() => {
    return [
      {
        text: 'larger',
        isError: cardData.p21.isError[0],
      },
      {
        text: 'that',
        isError: cardData.p21.isError[1],
      },
      {
        text: 'communities',
        isError: cardData.p21.isError[2],
      },
      {
        text: 'promoted',
        isError: cardData.p21.isError[3],
      },
      {
        text: 'cooperation',
        isError: cardData.p21.isError[4],
      },
    ];
  }, [cardData.p21.isError]);

  const answer = 'larger communities that promoted cooperation';

  const solution = [0, 2, 1, 3, 4];
  ...
  const questionInfo: IQuestionProps = {
    ...
    mark: getMarking(cardData.p21.isSubmitted, cardData.p21.isCorrect),
  };

  const contents: ITranslation[] = [
    {
      en: ['Our ancestors lived in ', ' and the free exchange of knowledge, while Neanderthals tended to live in smaller groups. '],
      ...
      if (userSubmissionList.length > 0) {
        setCardData(prev => ({
          ...
          [pageNumber]: {
            ...
            isSubmitted,
            isCorrect: userSubmissionList[0].isCorrect || cardData.p21.isCorrect,
            isError: isSubmitted && checkAnswer(userSubmissionList[0].inputData[0]?.value || cardData.p21.clickedChipButtons),
          },
        }));
      }
  ...
  const handleSubmit = useCallback(() => {
    if (cardData[pageNumber].isSubmitted) {
      ...
      const isError = checkAnswer(cardData.p21.clickedChipButtons);
      const isCorrect = !isError.some(item => item);

      setCardData(prev => ({ ...prev, p21: { ...prev.p21, isSubmitted: true, isCorrect, isError } }));
      ...
      submitDataWithResult('P21', userSubmission, isCorrect);
    }
  }, [cardData.p21.clickedChipButtons, cardData.p21.isSubmitted]);
  ...
  return (
    ...
    >
      <Box marginBottom='24px' height='400px' useRound background='white' padding='10px'>
        ...
        <Box height='120px'>
          <Typography size={EStyleFontSizes.MEDIUM}>
            {contents[0].en[0]}
            <Typography type='blank' width='540px' title='빈칸' boxColor='var(--color-black)'></Typography>
            {contents[0].en[1]}
          </Typography>
        </Box>
      </Box>
