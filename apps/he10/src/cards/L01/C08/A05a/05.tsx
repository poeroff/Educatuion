...
const P05 = () => {
  ...
  const submitBtnColor = cardData.p05.isSubmitted
    ...
    : !(cardData.p05.answer1 === 'O' || isNotEmptyString(cardData.p05.answer2))
  ...
  const handleSubmit = () => {
    if (cardData.p05.isSubmitted) {
      ...
      setIsCorrect(
        cardData.p05.answer1.toLowerCase().trim() === cardData.p05.solution1 && cardData.p05.answer2.toLowerCase().trim() === cardData.p05.solution2,
      );
      setCardData(prev => ({
        ...prev,
        p05: {
          ...
          isSubmitted: true,
          isCorrect:
            cardData.p05.answer1.toLowerCase().trim() === cardData.p05.solution1 &&
            cardData.p05.answer2.toLowerCase().trim() === cardData.p05.solution2,
        },
      }));
  ...
  const handleInputChangeEvent = ({ target }: ChangeEvent<HTMLInputElement>) => {
    setCardData(prev => ({ ...prev, p05: { ...prev.p05, answer2: target.value } }));
    changeData('P05', 1, 2, target.value);
  };
  useEffect(() => {
    if (inputRef.current != null) {
      if (cardData.p05.answer1 === 'O') {
        setCardData(prev => ({ ...prev, p05: { ...prev.p05, answer2: '' } }));
        changeData('P05', 1, 2, '');
        inputRef.current.value = '';
        inputRef.current.disabled = true;
      } else if (cardData.p05.answer1 === 'X') {
  ...
  return (
    <Container
      ...
      submitDisabled={!(cardData.p05.answer1 === 'O' || isNotEmptyString(cardData.p05.answer2))}
        ...
        <Box marginTop='20px'>
          <Box hAlign={'flex-start'}>
            <StyledTypography>
              ...
              about 1,900 meters above sea level, the peak of Mt. Halla offers a scenic view of Jeju.
            </StyledTypography>
          </Box>
        </Box>
      </Box>
              ...
              <TD width='10px' color={EStyleTableTypes.GRAY}>
                <Box display='flex'>
                  ...
                  <Dropdown
                    type={'up'}
                    width='190px'
                    selectedValue={cardData.p05.answer1}
                    dropdownList={dropdownList}
                    isOpen={openDropdown[-1]}
                    readOnly={cardData.p05.isSubmitted}
                    isError={cardData.p05.isSubmitted && !isAnswer(cardData.p05.answer1, cardData.p05.solution1)}
                    ariaLabel={'5번 문제의 O/X 선택란'}
