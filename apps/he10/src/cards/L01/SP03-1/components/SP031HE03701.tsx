...
const SP031HE03701 = ({ headerText, questionText, word, choices, answer, pageNumber, store = 'SP03-1' }: IHE01401) => {
  ...
  const isCorrect = useMemo(() => {
    if (choices) {
      ...
      return cardData[pageNumber].answer.replace(/\s+/g, ' ').trim().toLowerCase() === answer;
    }
              ...
              <Input
                ...
                status={
                  cardData[pageNumber].answer.trim()
                    ? cardData[pageNumber].isSubmitted && cardData[pageNumber].answer.replace(/\s+/g, ' ').trim().toLowerCase() !== answer
                      ? InputStatus.ERROR
                      : InputStatus.ENABLE
                    : InputStatus.DEFAULT
                }
