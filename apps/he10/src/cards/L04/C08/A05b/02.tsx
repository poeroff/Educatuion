...
const P02 = () => {
  ...
  const problemInfo = {
    dialog: '2. Andrew는 규칙적인 운동뿐만 아니라 건강한 식단을 유지합니다.',
    questions: ['Andrew', 'a healthy diet.'],
    candidates: ['exercises regularly', 'not only', 'but also', 'maintains'],
    delimiter: ', ',
  };
  ...
  const nodeData: IContentList[] = [
    {
      children: (
        <Box marginTop='24px'>
          ...
          <Box marginLeft={'38px'} marginTop={'10px'}>
            <Typography>{problemInfo?.questions[1]}</Typography>
          </Box>
        </Box>
      ),
    },
