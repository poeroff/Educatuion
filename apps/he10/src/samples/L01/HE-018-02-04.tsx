...
import { Dialog, EStyleFontSizes, Image, Input, PinchZoom, Typography } from '@maidt-cntn/ui';
...
const HE0180204 = () => {
  ...
  return (
    <Container headerInfo={headerInfo} questionInfo={questionInfo} useExtend submitLabel='채점하기' onSubmit={() => {}}>
      <Box hAlign='flex-end' vAlign='flex-start' marginBottom='8px' position='absolute' zIndex={9} right={0} top={0}>
      ...
      <Box useFull>
        <PinchZoom>
          <Image src={imgSrc} width='820px' height='410px' style={{ margin: '0 auto' }} />
          <Box type='hidden'>{imgAlt}</Box>
        </PinchZoom>
        <Box position='absolute' bottom='30px' right='60px'>
          <Input width='244px' placeholder='내용을 넣어 주세요.' ariaLabel='답 입력란' />
        </Box>
      </Box>
