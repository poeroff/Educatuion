...
const ChatBubble = ({ text, image, direction, iconRef, onClickClose, width }: Props) => {
...
const Container = styled.div<{ direction: Direction; position: DOMRect; bubbleHeight: number; width?: string }>`
  ...
  align-items: center;
  position: fixed;
  top: ${props => `${props.position.bottom - 10}px`};
  left: ${props => `${props.position.right}px`};
  transform: ${props =>
    props.direction === 'top'
      ? `translate(calc(-50% - ${props.position.width / 2}px), calc(${props.position.height}px + 10px))`
