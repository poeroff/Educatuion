...
const P04 = () => {
  ...
  const isInputComplete = useMemo(() => isNotEmptyString(cardData.p04.input), [cardData.p04.input]);
  const isCorrect = useMemo(() => cardData.p04.input === answer, [cardData.p04.input]);

  useEffect(() => {
    if (cardData.p04.isSubmitted) {
      setMark(isCorrect ? 'correct' : 'incorrect');
    }
  }, [cardData.p04.isSubmitted, isCorrect]);
    ...
    if (pageId) {
      ...
      if (userSubmissionList.length > 0) {
        setCardData(prev => ({
          ...
          p04: {
            ...prev.p04,
            input: userSubmissionList[0].inputData[0]?.value || cardData.p04.input,
            isSubmitted,
          },
        }));
      }
        ...
        <Box padding='24px 48px' type='dashed' hAlign='center' flexDirection='column' useRound>
          <Box>
            ...
            <Input
              ...
              maxLength={6}
              value={cardData.p04.input}
              status={cardData.p04.isSubmitted && getInputStatus(isCorrect, cardData.p04.input)}
