...
const P05 = () => {
  const exampleAnswer =
    '(예시 답안)\n- 문제 : 이번 달 쓰담 달리기에서 페트병을 324개, 유리병을 109개 주웠습니다. 이번 달에 주운 페트병은 유리병보다 몇 개 더 많나요?\n- 식: 324-109=215\n- 답: 215개';
  ...
  const tmpSaveCanvas = async () => {
    if (!canvasRef.current?.isCanvasBlank()) {
      ...
      if (canvasDataURL) {
        setCardData(prev => ({ ...prev, p05: { ...prev.p05, canvasDataURL: canvasDataURL } }));
      ...
      if (userSubmissionList.length > 0) {
        setCardData(prev => ({
          ...
          p05: {
            ...
            canvasDataURL: userSubmissionList[0].inputData[2]?.value || cardData.p05.canvasDataURL,
            isSubmitted,
          },
        }));

        if (userSubmissionList[0].inputData[2]?.value || cardData.p05.canvasDataURL) {
          ...
          else canvasRef.current?.settingCanvasImageWithBlobs(dataURLToBlob(cardData.p05.canvasDataURL));
        }
      }
  ...
  const handleChangeInputs = (e: ChangeEvent<HTMLInputElement>, subKey: number) => {
    ...
    setCardData(prev => ({ ...prev, p05: { ...prev.p05, [name]: value } }));
    changeData(PAGE, 1, subKey, value);
  };

  const handleSubmit = async () => {
    if (!cardData.p05.isSubmitted) {
      const data = cardData.p05;
      const canvasDataURL = canvasRef.current?.getValue();
      if (canvasDataURL) {
        setCardData(prev => ({ ...prev, p05: { ...prev.p05, canvasDataURL: canvasDataURL, isSubmitted: true } }));
      }
      ...
      submitDataWithResult(PAGE, userSubmission, true);
    }
  };
  ...
  useEffect(() => {
    if (!cardData.p05.isSubmitted && (cardData.p05.answer1 === '' || cardData.p05.answer2 === '')) {
  ...
  return (
    ...
    >
      <Box vAlign='flex-start' alignItems='center'>
        ...
        <Typography>이전 표에서 두 수를 골라 뺄셈 문제를 만들고 해결해 보세요.</Typography>
      </Box>
      <Box width='100%' height='220px' marginTop='24px'>
        <Drawing ref={canvasRef} tmpSave={tmpSaveCanvas} disabled={cardData.p05.isSubmitted} />
      </Box>
