...
const P07 = () => {
        ...
        <Box>
          <Box vAlign='center' marginTop={'5px'}>
            <Box marginLeft='5px' marginRight='5px' hAlign='center'>
              <SvgIcon type={ESvgType.IMG} alt='빈칸' src={empty_square} size='40px' />
            </Box>
            <Typography fontSize='25px'>안에 알맞은 수를 써넣으세요.</Typography>
          </Box>
        </Box>
      </>
    ),
  ...
  const handleSubmit = () => {
    ...
    const isAnswer1 = checkAnswers(cardData.p07.answer1, cardData.p07.solution1[0]);
    const isAnswer2 = checkAnswers(cardData.p07.answer1, cardData.p07.solution1[1]);
    const isAnswer3 = checkAnswers(cardData.p07.answer2, cardData.p07.solution2[0]);
    const isAnswer4 = checkAnswers(cardData.p07.answer2, cardData.p07.solution2[1]);
    ...
    setCardData(prev => ({ ...prev, p07: { ...prev.p07, isSubmitted: true, isCorrect1, isCorrect2, isAllCorrect } }));
        ...
        inputData: [
          {
            subKey: 1,
            type: 'TEXT',
            value: cardData.p07.answer1[0],
          },
          {
            subKey: 2,
            type: 'TEXT',
            value: cardData.p07.answer1[1],
          },
          {
            subKey: 3,
            type: 'TEXT',
            value: cardData.p07.answer1[2],
          },
          {
            subKey: 4,
            type: 'TEXT',
            value: cardData.p07.answer1[3],
          },
          {
            subKey: 5,
            type: 'TEXT',
            value: cardData.p07.answer1[4],
          },
          {
            subKey: 6,
            type: 'TEXT',
            value: cardData.p07.answer1[5],
          },
          {
            subKey: 7,
            type: 'TEXT',
            value: cardData.p07.answer2[0],
          },
          {
            subKey: 8,
            type: 'TEXT',
            value: cardData.p07.answer2[1],
          },
          {
            subKey: 9,
            type: 'TEXT',
            value: cardData.p07.answer2[2],
          },
          {
            subKey: 10,
            type: 'TEXT',
            value: cardData.p07.answer2[3],
          },
          {
            subKey: 11,
            type: 'TEXT',
            value: cardData.p07.answer2[4],
          },
          {
            subKey: 12,
            type: 'TEXT',
            value: cardData.p07.answer2[5],
          },
        ],
  ...
  const handleChange = (key: number, index: number, value: string) => {
    if (key === 1) {
      const inputAnswer = [...cardData.p07.answer1];
      inputAnswer[index] = value;
      setCardData(prev => ({ ...prev, p07: { ...prev.p07, answer1: inputAnswer } }));
    } else if (key === 2) {
      const inputAnswer = [...cardData.p07.answer2];
      inputAnswer[index - 6] = value;
      setCardData(prev => ({ ...prev, p07: { ...prev.p07, answer2: inputAnswer } }));
    }
    changeData('P07', 1, index + 1, value);
  };

  const handleInputStatus = (key: number, index: number): InputStatus => {
    if (key === 1) {
      return !isNotEmptyString(cardData.p07.answer1[index])
        ? InputStatus.DEFAULT
        : cardData.p07.isSubmitted && !cardData.p07.isCorrect1
        ? InputStatus.ERROR
        : InputStatus.ENABLE;
    }
    return !isNotEmptyString(cardData.p07.answer2[index])
      ? InputStatus.DEFAULT
      : cardData.p07.isSubmitted && !cardData.p07.isCorrect2
      ? InputStatus.ERROR
      : InputStatus.ENABLE;
  };

  const isInputAnswer = () => {
    const answerList1 = [...cardData.p07.answer1];
    const answerList2 = [...cardData.p07.answer2];
  ...
  return (
    ...
    >
      <Box display='flex' hAlign='center'>
        <Box background='yellow' minWidth='200px' useRound hAlign='center'>
      ...
      <BottomSheet bottomSheetTargetId='targetContainer7' show={isShow} height='30%'>
