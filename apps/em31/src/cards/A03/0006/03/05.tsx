...
const P05 = () => {
  ...
  const handleClickTrainButton = (index: number) => {
    ...
    const newTrain = [...cardData.p05.answer1];
    newTrain[index] = clickedAnimalIndex;

    setCardData(prev => ({ ...prev, p05: { ...prev.p05, answer1: newTrain } }));
    changeData('P05', 1, 1, cardData.p05.answer1);

    setClickedAnimalIndex(-1);
  };

  const setSubmitBtnColor = () => {
    if (cardData.p05.answer1 && cardData.p05.answer1.every(val => val !== -1)) {
      if (isShow && cardData.p05.isSubmitted) {
  ...
  const setSubmitDisabled = () => {
    if (cardData.p05.answer1 && cardData.p05.answer1.every(val => val !== -1)) return false;
    else return true;
  };
  ...
  const handleSubmit = () => {
    setClickedAnimalIndex(-1);
    setClickedTrainIndex(-1);
    const isCorrect = cardData.p05.solution1.every(answer => cardData.p05.answer1.includes(answer));
  ...
  const setTrainAriaLabel = () => {
    if (cardData.p05.answer1.every(val => val === -1)) return '파란색 기차';

    let labelText = '파란색 기차에 ';
    for (let i = 0; i < 4; i++) {
      if (cardData.p05.answer1[i] !== -1) {
        labelText += animalCardTData[cardData.p05.answer1[i]].animalName + ' ';
      }
    }
  ...
  return (
    <Container
      ...
      submitLabel={cardData.p05.isSubmitted ? (isShow ? '답안 닫기' : '답안 보기') : '완료하기'}
          ...
          {animalCardTData.map((value, index) => {
            return (
              <AnimalCard
                ...
                state={cardData.p05.answer1.includes(index) ? 'disabled' : clickedAnimalIndex === index ? 'active' : undefined}
                disabled={cardData.p05.isSubmitted || cardData.p05.answer1.includes(index)}
                aria-label={
                  `${value.animalName} (${value.expression.replace(/=/g, '')})` +
                  (clickedAnimalIndex === index ? ', 선택됨' : cardData.p05.answer1.includes(index) ? ', 선택 불가능' : ', 선택 가능')
                }
                title='엔터키를 입력하고 원하는 파란색 기차 칸을 선택해주세요.'
              >
        ...
        <Box position='absolute' bottom='37px' left='-40px' right='-40px'>
          <Box zIndex={1} position='relative' display='flex' alignItems='center' justifyContent='center'>
            <Box marginBottom='20px' position='relative'>
              <Image src='/A03/0006/03/A-EM31-030006-0305.png' alt={setTrainAriaLabel()} width='623px' height='120px' />
              <Box position='absolute' left='92px' top='38px' display='flex'>
                <HiddenText>기차의 몫은</HiddenText>7
              </Box>
            </Box>

            <Box position='absolute' left='401px' top='37px' display='flex' gap={61}>
              {cardData.p05.answer1.map((value, index = 1) => (
                <TrainButton
                  key={`train-item-${index}`}
                  isClick={index === clickedTrainIndex}
                  onClick={() => handleClickTrainButton(index)}
                  aria-label={`파란색 기차의 ${index + 1}번째 칸` + (value !== -1 ? `, ${animalCardTData[value].animalName} 동물이 있어요` : '')}
      ...
      <BottomSheet bottomSheetTargetId='targetContainer' marginTop={180} height='40%' show={isShow}>
        <Box background='lightGray' borderRadius='12px' marginTop='48px'>
          <Box>
            ...
            <Box marginTop={10}>
              <Image src='/A03/0006/03/A-EM31-030006-0305.png' height='100px' width='486px' alt='파란 기차에 얼룩말, 사슴, 기린, 토끼가 있습니다.' />
              <Box zIndex={1} left={165} top={590} position='absolute' display='flex' gap={49}>
                <SvgIcon type={ESvgType.IMG} src={zebra} width='45px' height='45px' />
                <SvgIcon type={ESvgType.IMG} src={deer} width='45px' height='45px' />
                <SvgIcon type={ESvgType.IMG} src={giraffe} width='45px' height='45px' />
                <SvgIcon type={ESvgType.IMG} src={rabbit} width='45px' height='45px' />
              </Box>
            </Box>
          </Box>
          <Box marginTop='48px'>
            ...
            <Box marginTop={10}>
              <Typography>얼룩말(14÷2=7), 사슴(28÷4=7), 기린(21÷3=7), 토끼(63÷9=7)입니다.</Typography>
            </Box>
          </Box>
        </Box>
      </BottomSheet>
