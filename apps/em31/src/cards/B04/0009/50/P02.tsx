...
const P02 = () => {
  ...
  const isBtnDisabled = () => {
    const answer1 = cardData.p02.answer1.value;
    const allZero = answer1 !== '';
    return allZero || cardData.p02.isSubmitted;
  };

  const submitAnswer = async () => {
    const correct = cardData.p02.answer1.value === ANSWER;
    ...
    if (canvasPath) {
      ...
      setCardData(prev => ({
        ...
        p02: {
          ...prev.p02,
          answer1: { ...prev.p02.answer1, isCorrect: correct },
  ...
  const answerInputChange = (value: string, num = 3) => {
    setCardData(prev => ({ ...prev, p02: { ...prev.p02, answer1: { ...prev.p02.answer1, value: value } } }));
    ...
    if (pageId) {
      ...
      if (userSubmissionList.length > 0) {
        setCardData(prev => ({
          ...
          p02: {
            ...prev.p02,
            canvasPath1: userSubmissionList[0].inputData[0]?.value || cardData.p02.canvasPath1,
            canvasDataURL: userSubmissionList[0].inputData[1]?.value || cardData.p02.canvasDataURL,
            answer1: {
              ...prev.p02.answer1,
              value: userSubmissionList[0].inputData[2]?.value.value || cardData.p02.answer1.value,
        ...
        if (!isSubmitted) {
          ...
          if (userSubmissionList[0].inputData[0]?.value || cardData.p02.canvasPath1) {
            canvasRef.current?.settingCanvasImage({
              subjectCode: subjectCode,
              uploadPath: userSubmissionList[0].inputData[0]?.value || cardData.p02.canvasPath1,
            });
          }
        }
      }
    ...
    >
      <Box display='flex' flexDirection='column' useFull>
        <Box useFull>
          ...
          <Box marginTop='30px'>
            <Mark size={'medium'} type={cardData.p02.isSubmitted ? (cardData.p02.isCorrect ? 'correct' : 'incorrect') : 'none'}>
              <Typography
                useGap={false}
            ...
            <Typography>지민이가 봉지에 담은 땅콩은 몇 개인지 구해 보세요.</Typography>
            <Box height={'220px'}>
            ...
            <Box display='flex' justifyContent='right' alignItems='center' marginTop='10px'>
              ...
              <Input
                ...
                status={inputStatus(cardData.p02.answer1.isCorrect)}
