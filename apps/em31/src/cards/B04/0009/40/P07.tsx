...
import AnswerSheet07 from './AnswerSheet07';

const P07 = () => {
  const CURRENT_PAGE = 'P07';
  const MAIN_KEY = 7;
  const ANSWER = ['8', '0'];
  ...
  const answerInputChange = (value: string, num: number) => {
    if (value.length > 0) {
      ...
      setCardData(prev => ({ ...prev, p07: { ...prev.p07, [`answer${num}`]: { value: value[value.length - 1] } } }));
      changeData(CURRENT_PAGE, MAIN_KEY, num, { value: _value });
    } else {
      setCardData(prev => ({ ...prev, p07: { ...prev.p07, [`answer${num}`]: { value: '' } } }));
      changeData(CURRENT_PAGE, MAIN_KEY, num, { value: '' });
    }
  };

  const isBtnDisabled = () => {
    const answer1 = cardData.p07.answer1.value;
    const answer2 = cardData.p07.answer2.value;
    const allZero = answer1 !== '' && answer2 !== '';
    return allZero || cardData.p07.isSubmitted;
  };
  ...
  const checkAnswerCorrect = useCallback(() => {
    const correct1 = cardData.p07.answer1.value === ANSWER[0];
    const correct2 = cardData.p07.answer2.value === ANSWER[1];
    setCardData(prev => ({
      ...prev,
      p07: {
        ...prev.p07,
        answer1: { ...prev.p07.answer1, isCorrect: correct1 },
        answer2: { ...prev.p07.answer2, isCorrect: correct2 },
      },
    }));
    const result = correct1 && correct2;
    setCardData(prev => ({ ...prev, p07: { ...prev.p07, isCorrect: result } }));
    return result;
  }, [cardData.p07]);
  ...
  const submitAnswer = () => {
        ...
        inputData: [
          {
            ...
            isCorrect: cardData.p07.answer1.value === ANSWER[0],
          },
          {
            subKey: 2,
            type: 'TEXT',
            value: cardData.p07.answer2,
            isCorrect: cardData.p07.answer2.value === ANSWER[1],
          },
        ],
        isCorrect: correct,
      },
    ...
    setCardData(prev => ({
      ...prev,
      p07: { ...prev.p07, isSubmitted: true, isCorrect: correct, inputData: userSubmission[0].inputData as [] },
    }));
  ...
  const inputStatus = (isCorrect: boolean) => {
    return cardData.p07.isSubmitted ? (isCorrect ? InputStatus.ENABLE : InputStatus.ERROR) : InputStatus.ENABLE;
  };
  ...
  return (
    <Container
      ...
      submitLabel={cardData.p07.isSubmitted ? (isShowBottom ? '답안 닫기' : '답안 보기') : '채점하기'}
      onSubmit={onSubmit}
      vAlign='flex-start'
      useRound
    >
              ...
              <TR>
                <TD>
                  <Input
                    ...
                    status={inputStatus(cardData.p07.answer2.isCorrect)}
                ...
                <TD>
                  <Input
                    ...
                    status={inputStatus(cardData.p07.answer1.isCorrect)}
