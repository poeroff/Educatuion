...
import AnswerSheet06 from './AnswerSheet06';

const P06 = () => {
  const CURRENT_PAGE = 'P06';
  const MAIN_KEY = 6;
  const ANSWER = ['6', '0', '2'];
  ...
  const answerInputChange = (value: string, num: number) => {
    if (value.length > 0) {
      ...
      setCardData(prev => ({ ...prev, p06: { ...prev.p06, [`answer${num}`]: { value: value[value.length - 1] } } }));
      changeData(CURRENT_PAGE, MAIN_KEY, num, { value: _value });
    } else {
      setCardData(prev => ({ ...prev, p06: { ...prev.p06, [`answer${num}`]: { value: '' } } }));
      changeData(CURRENT_PAGE, MAIN_KEY, num, { value: '' });
    }
  };

  const isBtnDisabled = () => {
    const answer1 = cardData.p06.answer1.value;
    const answer2 = cardData.p06.answer2.value;
    const answer3 = cardData.p06.answer3.value;
    const allZero = answer1 !== '' && answer2 !== '' && answer3 !== '';
    return allZero || cardData.p06.isSubmitted;
  };

  const onSubmit = () => {
    if (!cardData.p06.isSubmitted) {
      setCardData(prev => ({ ...prev, p06: { ...prev.p06, isSubmitted: true } }));
      submitAnswer();
    } else {
  ...
  const checkAnswerCorrect = useCallback(() => {
    const correct1 = cardData.p06.answer1.value === ANSWER[0];
    const correct2 = cardData.p06.answer2.value === ANSWER[1];
    const correct3 = cardData.p06.answer3.value === ANSWER[2];
    setCardData(prev => ({
      ...prev,
      p06: {
        ...prev.p06,
        answer1: { ...prev.p06.answer1, isCorrect: correct1 },
        answer2: { ...prev.p06.answer2, isCorrect: correct2 },
        answer3: { ...prev.p06.answer3, isCorrect: correct3 },
      },
    }));
    const result = correct1 && correct2 && correct3;
    setCardData(prev => ({ ...prev, p06: { ...prev.p06, isCorrect: result } }));
    return result;
  }, [cardData.p06]);
  ...
  const submitAnswer = () => {
        ...
        inputData: [
          {
            ...
            isCorrect: cardData.p06.answer1.value === ANSWER[0],
          },
          {
            subKey: 2,
            type: 'TEXT',
            value: cardData.p06.answer2,
            isCorrect: cardData.p06.answer2.value === ANSWER[1],
          },
          {
            subKey: 3,
            type: 'TEXT',
            value: cardData.p06.answer3,
            isCorrect: cardData.p06.answer3.value === ANSWER[2],
          },
        ],
        isCorrect: correct,
      },
    ...
    setCardData(prev => ({
      ...prev,
      p06: { ...prev.p06, isSubmitted: true, isCorrect: correct, inputData: userSubmission[0].inputData as [] },
    }));
  ...
  const inputStatus = (isCorrect: boolean) => {
    return cardData.p06.isSubmitted ? (isCorrect ? InputStatus.ENABLE : InputStatus.ERROR) : InputStatus.ENABLE;
  };
  ...
  return (
    <Container
      ...
      submitLabel={cardData.p06.isSubmitted ? (isShowBottom ? '답안 닫기' : '답안 보기') : '채점하기'}
      onSubmit={onSubmit}
      vAlign='flex-start'
      useRound
    >
            ...
            <TFoot>
              <TR>
                <TD>
                  <Input
                    type='number'
                    value={cardData.p06.answer3.value}
                    status={inputStatus(cardData.p06.answer3.isCorrect)}
                ...
                <TD>
                  <Input
                    ...
                    status={inputStatus(cardData.p06.answer2.isCorrect)}
                ...
                <TD>
                  <Input
                    ...
                    status={inputStatus(cardData.p06.answer1.isCorrect)}
