...
const P01 = () => {
  ...
  const [cardData, setCardData] = useRecoilState(C01_0005_50);
  ...
  const questionInfo: IQuestionProps = {
    ...
    text: (
      <>
        <Label value={1} type='icon' size='small' />수 모형을 보고 478-235 를 구해 보세요 .
      </>
    ),
    ...
    >
      <Box useFull vAlign='center' flexDirection='column'>
        <Box type='line' padding='20px 40px' useRound>
          <Image
            src='/C01/0005/50/DEC311003.png'
            alt='백 모형 4 개 , 십
모형 7 개 , 일 모형 8 개가
있습니다 . 이 중 백 모형 2 개 ,
십 모형 3 개 , 일 모형 5 개에 X
표가 있습니다 '
            width='400px'
            height='200px'
          />
      ...
      <BottomSheet bottomSheetTargetId='targetContainer' height='30%' show={isShow} closeOption={{ useYn: true, onClose: () => setIsShow(false) }}>
        <Box background='lightGray' borderRadius='12px' marginTop='35px'>
          ...
          <Box marginTop='12px'>
            수 모형 478에서 235만큼 X표 하고 남은 수 모형을 세어 보면 백 모형이 2개, 십 모형이 4개, 일 모형이 3개이므로 478-235=243입니다.
          </Box>
        </Box>
      </BottomSheet>
    </Container>
