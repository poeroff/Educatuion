...
import { C01_0005_40 } from './store';

const P01 = () => {
  const [isShow, setShow] = useState<boolean>(false);
  const [cardData, setCardData] = useRecoilState(C01_0005_40);
          ...
          row={({ value, index = 1 }) => (
            <Box vAlign='center' justifyContent='center' key={index}>
              {cardData.p01.isSubmitted && index - 1 === radio ? (
                <Box background='red' padding='16px 24px' useRound marginRight='24px'>
      ...
      >
        <Box background='lightGray' borderRadius='12px' marginTop='48px' padding='28px'>
          ...
          <Box position='relative' marginTop='40px'>
            ...
            <Box display={'flex'} flexDirection={'column'} marginTop='12px' gap={'20px'}>
              <Typography>{`826-413=413, 593-121=472`}</Typography>
              <Typography>{`=>413<472`}</Typography>
            </Box>
          </Box>
        </Box>
      </BottomSheet>
