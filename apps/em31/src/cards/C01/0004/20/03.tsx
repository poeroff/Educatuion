...
const P03 = () => {
  ...
  const isAnswer1Correct = useMemo(() => cardData.p03.input1.join('') === answers[0], [cardData.p03.input1]);
  ...
  }, [cardData.p03.isSubmitted, isAllCorrect]);
    ...
    if (pageId) {
      ...
      if (userSubmissionList.length > 0) {
        setCardData(prev => ({
          ...
          p03: {
            ...prev.p03,
            input1: userSubmissionList[0].inputData[0]?.value || cardData.p03.input1,
            isSubmitted,
          },
        }));
      }
  ...
  const handleListInputChange = (index: number, subKey: 1, e: React.ChangeEvent<HTMLInputElement>) => {
    const newInput = [...cardData.p03[`input${subKey}`]];
    newInput[index] = e.target.value;

    setCardData(prev => ({ ...prev, p03: { ...prev.p03, [`input${subKey}`]: newInput } }));
  ...
  const isInputComplete = useMemo(() => {
    return cardData.p03.input1.every(value => value !== '');
  }, [cardData.p03.input1]);
  ...
  return (
    ...
    >
      <Box hAlign='start' justifyContent='flex-start' flexDirection='column'>
        <QuestionBox>
          <Typography weight={'var(--font-weight-bold)'} ariaLabel='첫번째 카드 : 829'>
            829
          </Typography>
          <Typography weight={'var(--font-weight-bold)'} ariaLabel='두번째 카드 : 387'>
            387
          </Typography>
        </QuestionBox>
        <Box marginTop='20px'>
          <Input
            ...
            ariaLabel='답을 입력하세요'
            maxLength={5}
            value={cardData.p03.input1[0]}
            onChange={e => handleListInputChange(0, 1, e)}
            status={
              !cardData.p03.isSubmitted ? InputStatus.ENABLE : cardData.p03.input1[0].trim() === '1216' ? InputStatus.ENABLE : InputStatus.ERROR
            }
