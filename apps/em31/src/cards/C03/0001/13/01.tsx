...
const P01 = () => {
  ...
  const [cardData, setCardData] = useRecoilState(C03_0001_13);
  ...
  const questionInfo: IQuestionProps = {
    ...
    text: (
      <>
        ...
        초콜릿이 한 상자에 8개씩 들어 있습니다. 6상자에 들어 있는 초콜릿 은 모두 몇 개인가요?
      </>
    ),
  ...
  const handleSubmit = () => {
    ...
    if (isSubmitted) {
      ...
      const isCorrectAll = answers.every((answer, index) => isEquationAnswer(answer, solutions[index]));
          ...
          inputData: [
            {
              ...
              isAnswer: true,
              isCorrect: isEquationAnswer(answers[0], solutions[0]),
            },
            {
              subKey: 2,
              type: 'TEXT',
              ...
              isAnswer: true,
              isCorrect: isEquationAnswer(answers[1], solutions[1]),
            },
          ],
  ...
  const getInputStatus = (index: number) => {
    ...
    if (isSubmitted) {
      return isEquationAnswer(answers[index], solutions[index]) ? InputStatus.ENABLE : InputStatus.ERROR;
    } else {
    ...
    >
      <Box useFull vAlign='center' flexDirection='column'>
        <Box type='line' padding='20px 40px' useRound>
          <Image src='/C03/0001/13/DEC313003.png' alt='초콜릿이 8개씩 들어 있는 상자 6개가 그려진 그림입니다.' width='800px' height='137px' />
        </Box>
