...
import { C03000442_store } from './store';
...
const P01 = ({ _page = 'P01', _store = C03000442_store }: Props) => {
  ...
  const questionInfo: IQuestionProps = {
    ...
    text: (
      <>
        ...
        <Typography useGap={false}>수 카드를 한 번씩 모두 이용하여 곱셈식을 만들고, 만든 곱셈식을 나눗셈식 2개로 나타내 보세요.</Typography>
      </>
    ),
  };
  ...
  return (
    <>
      ...
      <Box display='flex' flexDirection='column' marginTop='24px'>
        <Box display='flex'>
          <TextChip text='곱셈식' width={125} marginRight={25} />
...
const SolutionNode = () => {
  const explains = [
    '수 카드 중에서 두 수를 곱해서 나올 수 있는 곱셈식은 2×8=16입니다.',
    '곱셈식 2×8=16을 나눗셈식 2개로 나타내면 16÷2=8, 16÷8=2입니다.',
  ];
  ...
  return (
    <>
      <AnswerTagBox marginTop={0} label='답안'>
        <Box>
          <Typography useGap={false}>2, 1, 6 /</Typography>
        </Box>
        <Box>
          <Typography useGap={false}>1, 6, 2, 8, 1, 6, 8, 2 (또는 1, 6, 8, 2, 1, 6, 2, 8)</Typography>
        </Box>
      </AnswerTagBox>
