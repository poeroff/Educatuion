...
const P05 = () => {
  ...
  const CONST: Props['Layout'] = {
    ...
    questionInfo: {
      text: '영상을 보고, 뛸 수 있는 친구들의 이름 카드를 빈칸에 옮겨 봅시다.',
      mark:
        isSubmittedInput(mainKey, 'TEXT-01') && isSubmittedInput(mainKey, 'TEXT-02')
    ...
    video: {
      src: '/L01/C02/A02/EE4-L01-C02-A02-P05.mp4',
      srt: `
      1
  ...
  const INITIAL: Props['INITIAL'] = {
    isValChk: getValueInputData(mainKey, 'TEXT-01') && getValueInputData(mainKey, 'TEXT-02') ? true : false,
    isAnswer: String(getCorrectData(mainKey)[0].inputDatas[0][0].value),
    isComplete: isSubmittedInput(mainKey, 'TEXT-01') && isSubmittedInput(mainKey, 'TEXT-02'),
  ...
  return (
    ...
    >
      <BoxWrap gap='10px' useFull>
        ...
        <Box vAlign={'center'} useFull width='532px'>
          <List>
            {[...Array(2)].map((item, idx) => {
              return (
                <CardBox key={idx} width='50%'>
                  <Cards
                    type={getValueInputData(mainKey, 'TEXT-0' + (idx + 1)) === '' ? 'answer' : 'exam'}
                    isCorrect={INITIAL.isValChk ? INITIAL.isComplete && !INITIAL.isCorrect : false}
                    onDragOver={e => {
