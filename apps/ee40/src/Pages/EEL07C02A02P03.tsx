...
const EEL07C02A02P03 = ({ layout, pageData, contentInfo }: Props) => {
  const CONST = { ...layout };
    ...
    return aInfo.answerImg.map((item, idx) => {
      ...
      return (
        <RadioBox
          ...
          gap={0}
        >
          <BoxWrap width={450} height={118}>
            <Label value={String(index)} size='middle' />
            <Image alt={item.answerImgSrc} src={item.answerImgSrc} width='264px' height='118px' style={{ objectFit: 'contain' }} />
