...
const EEL04C02A02P04 = ({ layout, pageData, contentInfo }: Props) => {
  const CONST = { ...layout };
    ...
    return aInfo.answerImg.map((item, idx) => {
      ...
      return (
        <RadioBox
          ...
          gap={0}
        >
          <BoxWrap width={450} height={158}>
            <Label value={String(index)} size='middle' />
            <Image alt={item.answerImgSrc} src={item.answerImgSrc} width='270px' height='158px' style={{ objectFit: 'contain' }} />

            <Box vAlign='center'>
              <Typography fontSize='32px' weight={500}>
                {item.text}
              </Typography>
            </Box>
          </BoxWrap>
  ...
  return (
    ...
    >
      <BoxWrap useFull {...{ marginTop: '10px' }}>
        <Box display='flex' gap='40px' justifyContent='center' alignItems='center' height={'100%'} marginBottom={'30px'} width={'100%'}>
          <Box hAlign={'center'} useFull height={'fit-content'} flex={1} width={480}>
            {/* 좌측 문제 영역 */}
            {questionArea()}
          </Box>
          <Box justifyContent='flex-start' flex={1} width={480}>
            {/* 우측 선택지 영역 */}
            {answerArea()}
          </Box>
        </Box>
