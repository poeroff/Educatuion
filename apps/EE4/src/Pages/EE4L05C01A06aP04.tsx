...
const EE4L05C01A06aP04 = ({ headerInfo, questionInfo, pageData }: PageProps) => {
            ...
            <Box width='406px'>
              ...
              {value?.data &&
                value?.data.map((value, index) => {
                  return (
                    <Box key={index} height='130px' paddingLeft='10px'>
                      <Label type='paint' value={value?.type} background={value?.color} />
                      <Typography weight={600} style={{ whiteSpace: 'nowrap' }}>
                        {value.question}
                      </Typography>
                      {isOpen && (
                        <Box {...{ margin: '10px 0 0 38px' }}>
                          <Typography weight={600} color='#2F38C7' style={{ whiteSpace: 'nowrap' }}>
                            {value.answer}
                          </Typography>
                        </Box>
                      )}
...
export default EE4L05C01A06aP04;
