...
import type { TContentInfo, TAnswerType, TAnswerInfo, TSolutionData } from '@/types/contentInfo';
...
interface Props {
  ...
  pageData: TPageData;
  contentInfo: TContentInfo<TAnswerType>;
}

const EEL03C04A04P01 = ({ layout, pageData, contentInfo }: Props) => {
  const CONST = { ...layout };
  const { pageNumber, mainKey, subKey, getDefaultData, getCorrectData, getSolutionData } = pageData;
  const { answerInfo } = contentInfo;
    ...
    return aInfo.answerImg.map((item, idx) => {
      ...
      return (
        ...
        >
          ...
          <Image alt={item.answerImgAlt} src={item.answerImgSrc} width='200px' height='262px' style={{ objectFit: 'contain' }} />
        </RadioBox>
      );
    });
  };
    ...
    >
      <BoxWrap useFull {...{ marginTop: '10px' }}>
        <Box
          display='flex'
          gap='30px'
          justifyContent={answerInfo.answerType === 'text' ? 'flex-start' : 'center'}
          alignItems={answerInfo.answerType === 'text' ? 'flex-start' : 'center'}
          height={'100%'}
          marginBottom={'30px'}
          width={'100%'}
        >
