...
const P05 = () => {
  ...
  const dropArr1 = ['Despite', 'Although'];
  ...
  const getSubmitLabel = () => (cardData.p05.isSubmitted ? (showAnswer ? '답안 닫기' : '답안 보기') : '채점하기');
  ...
  const getDropDownSelectedValue = () => {
    return !cardData.p05.isSubmitted && !cardData.p05.answer ? '' : cardData.p05.answer;
  };
...
두 번째 문장은 다음과 같다: 첫 번째 조각: "Despite" (파란색 글씨로 작성됨) 두 번째 조각: "challenges in their lives," 세 번째 조각: "the artists never gave up on their art." 이 조각들이 합쳐져서 "Despite challenges in their lives, the artists never gave up on their art."라는 문장이 된다.`}
        ...
        <Box marginTop='20px'>
          <Box display='flex' alignItems='center' flexWrap='wrap'>
            ...
            <Dropdown
              ...
              isError={cardData.p05.isSubmitted ? cardData.p05.isCorrect !== true : false}
              onClick={value => handleDropdownClick(value)}
            />
            <Typography style={{ whiteSpace: 'normal' }}>her writing was not well-organized, </Typography>
            <Typography style={{ whiteSpace: 'normal' }}>she got an A because of the informative content.</Typography>
          </Box>
        </Box>
      </Box>
      <BottomSheet
      ...
      >
        <Box background='lightGray' borderRadius='12px' marginTop='48px'>
          ...
          <Box marginTop='12px'>
            <Typography useGap={false}>{cardData.p05.solution}</Typography>
          </Box>
        </Box>
      </BottomSheet>
    </Container>
