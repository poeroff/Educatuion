...
const P02 = () => {
  ...
  const getSubmitLabel = () => (cardData.p02.isSubmitted ? (showAnswer ? '답안 닫기' : '답안 보기') : '채점하기');
...
두 번째 문장은 다음과 같다: 첫 번째 조각: "Despite" (파란색 글씨로 작성됨) 두 번째 조각: "challenges in their lives," 세 번째 조각: "the artists never gave up on their art." 이 조각들이 합쳐져서 "Despite challenges in their lives, the artists never gave up on their art."라는 문장이 된다.`}
        ...
        <Box marginTop='20px'>
          <Box display='flex' alignItems='center' flexWrap='wrap'>
            ...
            <Typography style={{ whiteSpace: 'normal' }}>John learned a lot about the local culture</Typography>
            <Dropdown
              type='up'
              ...
              isError={cardData.p02.isSubmitted ? cardData.p02.isCorrect !== true : false}
              onClick={value => handleDropdownClick(value)}
            />
            <Typography style={{ whiteSpace: 'normal' }}>his stay in Andong Hahoe Village.</Typography>
          </Box>
        </Box>
      </Box>
      <BottomSheet
