...
const P21 = () => {
  ...
  const contents: ITranslation[] = [
    {
      en: ', her talent and imagination were both limitless. ',
      ko: '비록 신체적 제약으로 인해 작은 오두막집에 갇혀 지냈지만, 그녀의 재능과 상상력은 무한했습니다.',
    },
  ];
  const [chipButtonInfo, setChipButtonInfo] = useState<IChipButtonInfo[]>([
    ...
    {
      text: 'her physical limitations',
      isError: false,
    },
  ]);

  const answer = 'Although her physical limitations confined her to a small cottage';
  ...
  const init = async () => {
    const pageId = pageIds.find(page => page.page === 'P21')?.pageId;
    ...
    if (pageId) {
      ...
      initData('P21', userSubmissionList, defaultSubmission, isSubmitted);
  ...
  const chipButtonOnClick = (index: number) => {
    ...
    if (clickedChipButtons.includes(index)) {
      ...
      changeData('P21', 1, 1, [...cardData.p21.answer, index]);
    }
  };
  ...
  return (
    ...
    >
      <Box marginBottom='24px' useRound height='167px' background='white' padding='10px'>
        <Scroll tabIndex={0}>
          <List<ITranslation>
            data={contents}
            row={({ value, index = 1 }) => (
