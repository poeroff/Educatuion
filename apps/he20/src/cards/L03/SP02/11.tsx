...
const P11 = () => {
  ...
  return (
    <Container
      ...
      submitLabel={!cardData.p11.isSubmitted ? '완료하기' : !isAnswerShow ? '답안 보기' : '답안 닫기'}
      onSubmit={onSubmit}
      submitDisabled={
    ...
    >
      <Box hAlign='start'>
        ...
        <Box backgroundColor='var(--color-blue-100)' border='none' useRound hAlign='center' padding='12px' margin='0 8px'>
          <Typography>Are there any rulls to follow in the classroom?</Typography>
        </Box>
      </Box>
      <Box marginTop='20px'>
        <Box hAlign='end'>
          ...
          <TextView title='보기' hAlign='start'>
            <Box>
              <Typography align='left' size={EStyleFontSizes['X-MEDIUM']}>
                Yes. You're not allowed to make loud noise during class.
              </Typography>
            </Box>
            <Box>
              <Typography align='left' size={EStyleFontSizes['X-MEDIUM']}>
                Of course not. You don't have to follow the rules.
              </Typography>
            </Box>
          </TextView>
        </Box>
      ...
      <BottomSheet bottomSheetTargetId='targetContainer' height='40%' show={isAnswerShow}>
        <Box background='lightGray' borderRadius='12px' marginTop='30px'>
          ...
          <Box marginTop='10px'>
            <Typography>Yes. You're not allowed to make loud noise during class.</Typography>
          </Box>
          ...
          <Box marginTop='10px' display='flex' flexDirection='column'>
            <Typography>A: 교실에서 지켜야 할 규칙이 있나요?</Typography>
            <Typography>B: 네. 여러분은 수업 중에 시끄러운 소리를 내서는 안 됩니다.</Typography>
          </Box>
        </Box>
      </BottomSheet>
    </Container>
