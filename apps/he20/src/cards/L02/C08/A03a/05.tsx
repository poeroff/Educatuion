...
const P05 = ({ imgSrc, imgAlt }: { imgSrc: string; imgAlt: string }) => {
  ...
  return (
    ...
    >
      ...
      <Box marginTop='24px'>
        <Box>
          <Typography>5. 교통 체증이 심해서 회의가 끝난 후에 도착했어. 우리는 지하철을 탔어야 했어.</Typography>
        </Box>
        <Box hAlign={'flex-start'}>
          <StyledTypography>
            <SvgIcon style={{ verticalAlign: 'text-top', padding: '4px 12px' }} src={arrow_right} size='36px' />
            <TextPartRight>Because the traffic was bad, we got to the meeting after it ended.</TextPartRight>
            <TextPartLeft2>We</TextPartLeft2>
            <Input
              width={'220px'}
              textAlign='start'
              value={answer}
            ...
            />
            <TextPartLeft>the subway.</TextPartLeft>
          </StyledTypography>
        </Box>
      </Box>
...
const TextPartLeft = styled.span`
  ...
  padding-left: 12px;
`;
const TextPartLeft2 = styled.span`
  display: inline;
