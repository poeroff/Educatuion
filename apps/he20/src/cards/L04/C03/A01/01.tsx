...
import { L04C03A01 } from './store';

const P01 = () => {
  const { changeData, initData, submitData, saveData } = usePageData();
  const pageIds = useRecoilValue(pageIdsAtom);
  const [cardData, setCardData] = useRecoilState(L04C03A01);
  ...
  const questionText = `How do you think technology will develop in the next 30 years?`;
    ...
    >
      <BoxWrap useFull>
        <Box hAlign={'center'} useFull>
          <Typography weight='500'>{questionText}</Typography>
        </Box>
        <Box useFull>
          <Textarea
            value={cardData.p01.answer1}
