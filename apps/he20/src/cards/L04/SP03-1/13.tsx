...
const P13 = ({ _page = 'P13' }: { _page?: string }) => {
  ...
  const contents: ITranslation[] = [
    {
      en: 'First, let’s explore',
      ko: '먼저 , 이 멋진 기술이 어떻게 적용되었는지 살펴봅시다.',
    },
  ];
  const [chipButtonInfo, setChipButtonInfo] = useState<IChipButtonInfo[]>([
    ...
    {
      text: 'this fascinating',
      isError: false,
    },
  ]);

  const answer = 'how this fascinating technology has been applied';
  ...
  return (
    ...
    >
      <Box marginBottom='24px' useRound height='137px' background='white' padding='10px' textAlign='center'>
        <Scroll tabIndex={0}>
          <List<ITranslation>
            data={contents}
            row={({ value, index = 1 }) => (
