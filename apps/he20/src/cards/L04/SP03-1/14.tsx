...
const P14 = ({ _page = 'P14' }: { _page?: string }) => {
  ...
  const contents: ITranslation[] = [
    {
      en: ['For instance, neural implants', 'like Parkinson’s disease.'],
      ko: '예를 들어 , 신경 임플란트는 파킨슨병과 같은 뇌 질환을 치료하는 데 사용됩니다.',
    },
  ];
  ...
  const answer = 'are used to treat brain disorders';
  ...
  const chipButtonOnClick = (index: number) => {
    ...
    if (clickedChipButtons.includes(index)) {
      ...
      changeData(_page.toUpperCase(), 1, 1, [...cardData.p14.answer, index]);
    }
  };
  ...
  return (
    ...
    >
      <Box marginBottom='24px' useRound height='170px' background='white' padding='10px' textAlign='left'>
        <Scroll tabIndex={0}>
          <List<ITranslation>
            data={contents}
            row={({ value, index = 1 }) => (
