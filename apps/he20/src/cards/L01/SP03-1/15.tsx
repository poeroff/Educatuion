...
const P15 = ({ _page = 'P15' }: { _page?: string }) => {
  ...
  const contents: ITranslation[] = [
    {
      en: 'I was impressed with the attention ',
      ko: '나는 직원들이 모든 동물에게 쏟는 애정이 인상 깊었다.',
    },
  ];
  ...
  const [chipButtonInfo, setChipButtonInfo] = useState<IChipButtonInfo[]>([
    ...
    {
      text: 'the staff members',
      isError: false,
    },
  ]);

  const answer = 'the staff members gave to all the animals';
  ...
  const chipButtonOnClick = (index: number) => {
    ...
    if (clickedChipButtons.includes(index)) {
      ...
      changeData(pageName, 1, 1, [...cardData.p15.answer, index]);
    }
  };
