...
const P14 = ({ _page = 'P14' }: { _page?: string }) => {
  ...
  const contents: ITranslation[] = [
    {
      en: 'While cleaning the habitats, we checked if ',
      ko: '서식지를 청소하는 동안 우리는 동물들에게 해를 끼칠 수 있는 위험 요소들이 있는지 확인했다.',
    },
  ];
  ...
  const answer = 'there were any hazards that could harm the animals';
  ...
  const chipButtonOnClick = (index: number) => {
    ...
    if (clickedChipButtons.includes(index)) {
      ...
      changeData(pageName, 1, 1, [...cardData.p14.answer, index]);
    }
  };
